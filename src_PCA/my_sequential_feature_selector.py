...
def _calc_score(selector, X, y, indices, **fit_params):
    #print('_calc_score>', '~~~~~~~~~~~~~~~~~~indices=', indices)
    #print('fit_params=',fit_params)
    #fit_params.update({'indices':indices,'cv':selector.cv})
    fit_params['indices'] = indices
    fit_params['cv'] = selector.cv
    #print('fit_params after update=',fit_params)
...
class SequentialFeatureSelector(BaseEstimator, MetaEstimatorMixin):
    ...
    def fit(self, X, y, custom_feature_names=None, **fit_params):
        ...
        # This weird-looking retrievel is necessary, since the parameter
        # 'fit_params' has a key called also 'fit_params'
        # but do not overwrite 'fit_params', since it is passed to the regressor's fit
        params = fit_params['fit_params']
        self.control_limit = params['control_limit']
        #print('sfs.fit> control_limit=', self.control_limit); quit()
        ...
        print('\n\n)))))))))forward', self.forward, 'k=', k, 'k_to_select=', k_to_select);#quit()
        try:
            while k != k_to_select:
                prev_subset = set(k_idx)

                #print('k=', k, 'k_idx=', k_idx, 'prev_subset=', prev_subset);pause()
                if self.forward:
    ...
    def _inclusion(self, orig_set, subset, X, y, ignore_feature=None,
        ...
        if remaining:
            ...
            print('_\n+++++++++++ Inclusion: best=', best, 'all_avg_scores=', all_avg_scores)
    ...
    def _exclusion(self, feature_set, X, y, fixed_feature=None, **fit_params):
        ...
        if n > 1:
            ...
            best = np.argmax(all_avg_scores)
            print('_\n-------------- Exlusion: best=', best, 'all_avg_scores=', all_avg_scores)
            res = (all_subsets[best],
                   all_avg_scores[best],
                   all_cv_scores[best])
        return res
    ...
    def set_param(self, reset=True, forward=True, floating=False, k_features=1):
        self.reset = reset
        self.forward = forward
        self.floating = floating
        self.k_features = k_features
