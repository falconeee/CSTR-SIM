...
def print_array(x, formatstr='%.2f', end='\n'):
    fprintf(np.array2string(x, formatter={'float_kind':lambda x: formatstr % x}), end=end)
...
def gen_train(Xtrain, Xtest, n_components, svd_solver='full', fault_num=None, featname=None, verbose=True):
    ...
    if verbose:
        fprintf('M2: l=', l, 'eigenvalues', 'sigma2noise=', sigma2noise, 'M2=\n', M_c_M2)
...
def detect_contrib(training):
    ...
    print('Xtrain_norm.shape=', X.shape, 'Xtil.shape=', Xtil.shape, 'SPE.shape=', SPE.shape,
...
####
def simulateX_pub1(faultdirections='Random', num_fault_directions=1, correlate=False,  no_graph=False,
					dropfigdir=None, figext='pgf', export_mat_file=False, show_faults=None):
    ...
    if faultdirections != 'Random':
        fprintf('simulateX_pub1> Fault directions not random! : ', faultdirections)
    ...
    for i in range(numsimfault):
        if faultdirections == 'Random':
            fdirs = np.random.choice(numsensors, num_fault_directions, replace=False)
            for k in range(num_fault_directions):
                ...
                if not show_faults is None:
                    if i in show_faults:
                        print('Fault i=', i, 'xi=', xi, 'f=%.3f' % f, 'signal after=%.3f' % signal[i][xi])#, file=sys.stderr)
            #f = np.random.random()*faultmagnitude
    ...
    if not no_graph:
        ...
        for s in range(numsensors):
            ...
            ax[s].axvline(x=numsim, color=(0.8,0.8,0.8,1), linestyle='-', linewidth=1.0, label='End of training')
            ax[s].set_ylim(-7, 7)
...
def simulateX_randomly(num_normal=1000, num_fault=2000, numvar=100, correlate=False):
    seed = 66649
    faultmagnitude = 5.0
    np.random.seed(seed=seed)
...
def contribution_and_time_plot(training, fault_data, fault_labels, fault_num=None, num_fault_directions=1,
        discard_first_test=None, fault_start=None, window=1,
        fault_detection_index='SPE',
        semilogy=False, benchmark=None, dropfigdir=None, figext='pgf'):
...
def TE_instantiate():
    ...
    rootdir = '/home/thomas/Nextcloud2/software/TE/Tennessee_Eastman/'
...
def main(fault_num):
    fprintf('Executing main(): ', 'fault_num=', fault_num, '...')   #; return; # quit()
    ...
    if not no_graph:
        contribution_plot_4_all_PC(training, fault_data, fault_num, discard_first_test=discard_first_test,
                        fault_start=pos, window=window, fault_detection_index=fault_detection_index,
                        featname=featname, verbose=False,
                        semilogy=semilogy, benchmark=benchmark, dropfigdir=dropfigdir)
        #quit()
        contribution_and_time_plot(training, fault_data, fault_labels, fault_num, num_fault_directions,
...
def test_multivar_fault_detection():
    ...
    if not no_graph:
        contribution_and_time_plot(training, fault_data, fault_labels, fault_num, num_fault_directions,
				discard_first_test=0, fault_start=special_fault_sample, window=1,
				fault_detection_index=fault_detection_index,
                semilogy=True, benchmark='pub1', dropfigdir=dropfigdir)
    #quit()
...
if __name__ == "__main__":
    ...
    #test_random_selection_CPU(); quit()
    #test_multivar_fault_detection(); quit()
    ...
    if len(sys.argv) > 1:
        ...
        #main(fault_num=fault_num)

    import timeit
    t = timeit.timeit("main(fault_num='01')", setup="from __main__ import main", number=1)
