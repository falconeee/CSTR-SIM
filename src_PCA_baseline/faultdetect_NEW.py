...
def simulateX_pub1(faultdirections='Random', num_fault_directions=1, correlate=False,  no_graph=False,
					dropfigdir=None, figext='pgf', export_mat_file=False):
    ...
    print('signal[1]=', signal[1], file=sys.stderr)
    ...
    for i in range(numsimfault):
        if faultdirections == 'Random':
            ...
            for k in range(num_fault_directions):
                ...
                # Special output
                if i==1:
                    print('i=', i, 'xi=', xi, 'f=', f, 'signal after=', signal[i][xi], file=sys.stderr)
            #f = np.random.random()*faultmagnitude
    ...
    if not no_graph:
        ...
        if not dropfigdir is None:
            ...
            #fname = '/home/thomas/Dropbox/papers/2020_Franklin/figs/sensorsignals' + '.' + figext    # DEBUG
...
def contribution_plot_4_all_PC(training, fault_data, fault_num=None,
    ...
    if not dropfigdir is None:
            ...
            #fname = '/home/thomas/Dropbox/papers/2020_Franklin/figs/test1.eps'    # DEBUG
...
def contribution_and_time_plot(training, fault_data, fault_labels, fault_num=None, num_fault_directions=1,
    ...
    if not dropfigdir is None:
        ...
        #fname = '/home/thomas/Dropbox/papers/2020_Franklin/figs/icontrib.pgf'
        #fname = '/home/thomas/Dropbox/papers/2020_Franklin/figs/test1.eps'    # DEBUG
    ...
    if not dropfigdir is None:
        ...
        #fname = '/home/thomas/Dropbox/papers/2020_Franklin/figs/test2.eps'    # DEBUG
        fname = '/home/thomas/Dropbox/papers/2020_Franklin/figs/tempevolsimu.pgf'
...
num_calls_b_and_b = 0
num_calls_J = 0
def select_b_and_b(training_data, n_components, svd_solver, fault_data,
                   index_pre_calculated=None, fault_detection_index=None,
    ...
    def b_and_b( level, B, leaf_visits, D, d, r, Xi, Xd, Psi, auxFeatSet, trace=False ):
        ...
        for j in range(r):
            ...
            if trace:
                fprintf('Psi=', feats(Psi[:r]), '{left_out=', 1+int(Psi[j]), '} J=', J)
                fprintf('\t$$$$$$ Calculated selection criterion with', cardAux, 'variables from set=', feats(auxFeatSet[:cardAux]), end='')
    ...
    J = M2 - B
    #print('M2=', M2, 'B=', B, 'J=', J)
    buf = 'Final unorderd best set after b&b with bound  J=%.3f %.2f=%s CPU time=%.3f' % (J, B, best_set+1, cputime)
    buf = buf + ' Fault number=' + fault_num
    fprintf(buf)
...
def select_exhaustive(training_data, n_components, svd_solver, fault_data, index_pre_calculated=None,
    ...
    for j in range(num_combinations):
        ...
        EM2 = calc_EM2_all_samples(fault_data, xmean, C, contributing)
        J[j] = M2 - EM2
        if J[j] > maxJ:
            maxJ = J[j]
            best_cand = cand
        ...
        if True:
            fprintf('\tJ[', '%6d' % (j+1), 'of %6d' % num_combinations, ']=',
                '%15.5f' % M2, '- %15.5f' % EM2, '=',
				'%15.5f' % J[j],
                '  cand+1= ', np.array(cand)+1, 'Best: J=', '%15.5f' % maxJ,
    ...
    for j in range(min(max_show,num_combinations)):
        ...
        if useLaTex_for_documentation and benchmark == 'pub1':
            if EM2 < control_limit:
                print('\\underline{%5.2f}' % EM2, ' & \\{', end='',  file=latexoutfile)
            else:
                print('%5.2f' % EM2, ' & \\{', end='',  file=latexoutfile)
...
def main(fault_num):
    ...
    if benchmark == 'pub1':
        ...
        k_select = 3            # exhausive search and b&b search
        fprintf('=================\n\tBenchmark: Qin paper\n==================\n')
    ...
    if do_b_and_b:
            contributing, cputime  = select_b_and_b(training_data, n_components, svd_solver, fault_data_window, mean_index_win,
...
def test_random_selection_CPU():
    ...
    training_data, training_labels, fault_data, fault_labels = simulateX_randoml (
    ...
    for k in range(1, max_k_select+1):
        ...
        contributing, cputimeBandB  = select_b_and_b(
            training_data, n_components, svd_solver, fault_data_window, mean_index_win,
                    fault_detection_index, fault_num, k_select=k_select, trace=False)
...
def test_multivar_fault_detection():
    ...
    no_graph = True
    training_data, training_labels, fault_data, fault_labels = simulateX_pub1(faultdirections,
			num_fault_directions, no_graph=no_graph, dropfigdir=dropfigdir)
    ...
    if not no_graph:
        contribution_and_time_plot(training, fault_data, fault_labels, fault_num, num_fault_directions,
				discard_first_test=0, fault_start=special_fault_sample, window=1, fault_detection_index=fault_detection_index,
    ...
    #for i in range(1946,1947):
        ...
        if is_fault:
            ...
            contributing, cputime = select_SBSSFS(training_data, n_components, svd_solver, x,
                Index, fault_detection_index, fault_num, contributing_preselection=None,
            ...
            # B & B
            contributing, cputime  = select_b_and_b(training_data, n_components,
                                                    svd_solver, x, Index, 
    ...
    print('SPECIAL: icontrib=', icontrib)
    #quit()
    ...
    for k in range(1,numvar):
        ...
        for k_b in range(0,k_f):
            ...
            final_xm_sequential = np.sort(bool2idxMissing(contributing))
            print('SPECIAL: Sequential  k_forward=', k_f, 'k_b=', k_b, 'final_xm_sequential=', final_xm_sequential)
    print('Closing ', fn, '...')
    specialoutfile.close()
